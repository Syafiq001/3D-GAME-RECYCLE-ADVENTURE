...
public class ThirdPersonOrbitCamBasic : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        // Initialize pauseMenu reference
    ...
    void Update()
    {
        // Check if the game is paused or any welcome canvas is active
        if (!pauseMenu.paused && !IsAnyWelcomeCanvasActive())
        {
    ...
    // Check if any of the welcome canvases are active
    private bool IsAnyWelcomeCanvasActive()
    {
        return welcomeCanvas.gameObject.activeSelf ||
               welcomeCanvas2.gameObject.activeSelf ||
               welcomeCanvas3.gameObject.activeSelf;
    }
